...
- 📊 **For informational queries (like standings, statistics, data, rankings):**
  1. First, search for the information using `search` or `search_documents`
  2. If search returns URLs, use `fetch_content` to get the actual data from the most relevant URL
  3. Extract the structured data (e.g., driver names and points for F1 standings)
  4. Create a Google Sheet with the extracted data using `create_google_sheet`:
     - Format: `title="Title"|column_headers=["Col1", "Col2"]|data=[["row1col1", "row1col2"], ["row2col1", "row2col2"]]`
     - Keep data arrays simple - max 20 rows to avoid parsing issues
  5. Send an email with the sheet link using `send_sheet_link` (use to="{RECEIVER_EMAIL}")
  6. After completing these steps, provide FINAL_ANSWER with a summary
- 🔁 Analyze that whether you have already got a good factual result from a tool, do NOT search again — but still create a sheet and send email if it's an informational query.
...
- ⏳ You have up to 5 steps available. Use them wisely to complete the full workflow for informational queries. Final step must end with FINAL_ANSWER.
